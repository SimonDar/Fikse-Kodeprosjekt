    ...
    .onUpdate(async (change, context) => {
      ...
      if (previousValue.status !== 'finish' && newValue.status === 'finish') {
        try {
          ...
          const timestamp = newValue.time.toDate();
          const formattedDate = timestamp.toLocaleDateString('no-NO', {
            day: 'numeric',
            month: 'long',
            year: 'numeric',
          ...
          const newEventLog = "finishMailSendt";
          const newEventTime = admin.firestore.FieldValue.serverTimestamp();
  
          // Update the document by setting the new values for event and eventTime
          await admin.firestore().collection('reparasjon').doc(docId).update({
            'eventTime': newEventTime,
            'events': admin.firestore.FieldValue.arrayUnion(newEventLog)
          });
